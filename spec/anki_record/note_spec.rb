...
RSpec.describe AnkiRecord::Note do
  ...
  describe "#save" do
    context "for a note that does not exist in the collection.anki21 database (custom note type, 2 card templates)" do
      before(:all) do
        @anki_package = AnkiRecord::AnkiPackage.new name: "package_to_test_notes"
        default_deck = @anki_package.collection.find_deck_by name: "Default"
        ...
        note.save
        @note_with_two_cards = note
        @note_record_data = @anki_package.prepare("select * from notes where id = #{@note_with_two_cards.id}").execute.first
        @cards_records_data = @anki_package.prepare("select * from cards where nid = #{@note_with_two_cards.id}").execute.to_a
      end
    ...
    context "for a note that does exist in the collection.anki21 database but has unsaved changes (custom note type, 2 card templates)" do
      before(:all) do
        @anki_package = AnkiRecord::AnkiPackage.new name: "package_to_test_notes"
        default_deck = @anki_package.collection.find_deck_by name: "Default"
        crazy_note_type = AnkiRecord::NoteType.new collection: @anki_package.collection, name: "crazy note type"
        ...
        note.crazy_back = "World"
        note.save
        @new_crazy_front = "What does the cow say?"
        @new_crazy_back = "Moo"
        note.crazy_front = @new_crazy_front
        note.crazy_back = @new_crazy_back
        note.save
        @already_saved_note_with_two_cards = note
        @note_record_data = @anki_package.prepare("select * from notes where id = #{@already_saved_note_with_two_cards.id}").execute.first
        @cards_records_data = @anki_package.prepare("select * from cards where nid = #{@already_saved_note_with_two_cards.id}").execute.to_a
      end
      let(:note_count) { @anki_package.prepare("select count(*) from notes;").execute.first["count(*)"] }
      let(:cards_count) { @anki_package.prepare("select count(*) from cards").execute.first["count(*)"] }
      let(:expected_number_of_notes) { 1 }
  ...
  describe "ghost method specs:" do
    subject(:note) do
      anki_package = AnkiRecord::AnkiPackage.new name: "package_to_test_notes"
      basic_note_type = anki_package.collection.find_note_type_by name: "Basic"
      default_deck = anki_package.collection.find_deck_by name: "Default"
      AnkiRecord::Note.new deck: default_deck, note_type: basic_note_type
    end
